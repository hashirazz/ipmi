...
void pantallaMudkip() {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("Puede reducir a añicos grandes rocas.\nDescansa enterrado en el lodo del lecho de los ríos.\nBatir su gran aleta caudal en el agua le permite nadar muy rápido.", cuadroIzqX + 10, 230, 250, 210);
  text("Altura: 0,4 m\nCategoría: Pez Lodo\nPeso: 7,5 kg\nHabilidad: Torrente\nTipo: agua\nDebilidad: Planta Eléctrico", cuadroDerX + 10, 230, 250, 210);
}

void pantallaSnorlax() {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("Este Pokémon es un glotón que lo único que hace aparte de comer es dormir.\nPuede ingerir hasta 400 kg de comida en un solo día.\nSus jugos gástricos pueden digerir cualquier toxina.", cuadroIzqX2 + 10, 230, 250, 210);
...
void pantallaEspeon() {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("Al predecir el siguiente movimiento de su rival, los extremos de su cola bífida tiemblan ligeramente.\nUsa el corto pelaje que cubre su cuerpo para sentir el viento y predecir las acciones enemigas.", cuadroIzqX3 + 10, 230, 250, 210);
  text("Altura: 0,9 m\nCategoría: Sol\nPeso: 26,5 kg\nHabilidad: Sincronía\nTipo: Psíquico\nDebilidad: Bicho, Fantasma, Siniestro", cuadroDerX3 + 10, 230, 250, 210);
}

void pantallaScolipede() {
  // PANTALLA 4
  if (enPausa4 == false) {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("Sujeta a sus presas con las garras que le sobresalen del cuello hasta inmovilizarlas y luego las remata con veneno.", cuadroIzqX4 + 10, 230, 250, 210);
  text("Altura: 2,5 m\nCategoría: Megaciempiés\nPeso: 200,5 kg\nHabilidad: Enjambre, Punto Tóxico\nTipo: Bicho, Veneno\nDebilidad: Fuego, Volador,\nRoca, Psíquico", cuadroDerX4 + 10, 230, 250, 210);
}

void pantallaMarshadow() {
  // PANTALLA 5
  if (enPausa5 == false) {
  ...
  textSize(16);
  textAlign(LEFT, TOP);
  text("Se oculta en la sombra de su oponente y copia sus movimientos y su fuerza.\nSu imitación resulta más poderosa que el original.\nSe infiltra en la sombra de las personas y los Pokémon. Sintoniza con sus sentimientos y copia sus capacidades.", cuadroIzqX5 + 10, 230, 250, 210);
